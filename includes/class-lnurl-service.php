...
class Nostr_Login_Pay_LNURL_Service {
    ...
    public function create_invoice_from_address( $lightning_address, $amount_msats, $comment = '' ) {
        ...
        if ( ! $this->is_valid_lightning_address( $lightning_address ) ) {
            return new WP_Error( 'invalid_address', __( 'Invalid Lightning address format', 'nostr-login-pay' ) );
        }
        ...
        if ( $amount_msats < $min_sendable || $amount_msats > $max_sendable ) {
            return new WP_Error(
                'amount_out_of_range',
                sprintf(
                    __( 'Amount must be between %d and %d sats', 'nostr-login-pay' ),
                    $min_sendable / 1000,
                    $max_sendable / 1000
                )
            );
        ...
        if ( empty( $lnurl_data['callback'] ) ) {
            return new WP_Error( 'no_callback', __( 'No callback URL provided by Lightning address', 'nostr-login-pay' ) );
        }
    ...
    private function fetch_lnurl_details( $lnurl_endpoint ) {
        ...
        if ( is_wp_error( $response ) ) {
            return new WP_Error(
                'lnurl_fetch_failed',
                __( 'Failed to fetch Lightning address details: ', 'nostr-login-pay' ) . $response->get_error_message()
            );
        }
        ...
        if ( $status_code !== 200 ) {
            return new WP_Error(
                'lnurl_error',
                sprintf( __( 'Lightning address returned error: HTTP %d', 'nostr-login-pay' ), $status_code )
            );
        }
        ...
        if ( ! $data || ! isset( $data['callback'] ) ) {
            return new WP_Error(
                ...
                __( 'Invalid response from Lightning address', 'nostr-login-pay' )
            );
        }
    ...
    private function request_invoice( $callback_url, $amount_msats, $comment = '' ) {
        ...
        if ( is_wp_error( $response ) ) {
            return new WP_Error(
                ...
                __( 'Failed to request invoice: ', 'nostr-login-pay' ) . $response->get_error_message()
            );
        }
        ...
        if ( $status_code !== 200 ) {
            return new WP_Error(
                'invoice_error',
                sprintf( __( 'Invoice request failed: HTTP %d', 'nostr-login-pay' ), $status_code )
            );
        }
        ...
        if ( ! $data || ! isset( $data['pr'] ) ) {
            return new WP_Error(
                ...
                __( 'Invalid invoice response', 'nostr-login-pay' )
            );
        }
