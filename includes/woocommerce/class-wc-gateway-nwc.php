...
class WC_Gateway_NWC extends WC_Payment_Gateway {
    ...
    public function __construct() {
        ...
        $this->method_title = __( 'Lightning (NWC)', 'nostr-login-pay' );
        $this->method_description = __( 'Accept Lightning payments via Nostr Wallet Connect', 'nostr-login-pay' );
        $this->supports = array(
            'products',
        );
        ...
        // Get settings
        $this->title = $this->get_option( 'title', __( 'Lightning Network (NWC)', 'nostr-login-pay' ) );
        $this->description = $this->get_option( 'description', __( 'Pay instantly with Bitcoin Lightning via NWC', 'nostr-login-pay' ) );
    ...
    public function init_form_fields() {
        ...
        $settings_url = admin_url( 'options-general.php?page=nostr-login-pay&tab=nwc' );
        
        $this->form_fields = array(
            'enabled' => array(
                'title' => __( 'Enable/Disable', 'nostr-login-pay' ),
                'type' => 'checkbox',
                'label' => __( 'Enable Lightning (NWC) payments', 'nostr-login-pay' ),
                'default' => 'yes',
            ),
            'title' => array(
                'title' => __( 'Title', 'nostr-login-pay' ),
                'type' => 'text',
                'description' => __( 'Payment method title that customers see during checkout', 'nostr-login-pay' ),
                'default' => __( 'Lightning Network', 'nostr-login-pay' ),
                'desc_tip' => true,
            ),
            'description' => array(
                'title' => __( 'Description', 'nostr-login-pay' ),
                'type' => 'textarea',
                'description' => __( 'Payment method description that customers see during checkout', 'nostr-login-pay' ),
                'default' => __( 'Pay instantly with Bitcoin Lightning Network via your NWC-enabled wallet', 'nostr-login-pay' ),
                'desc_tip' => true,
            ),
            'merchant_wallet_status' => array(
                'title' => __( 'Payment Methods', 'nostr-login-pay' ),
                'type' => 'title',
                'description' => sprintf(
                    __( '%s<br><a href="%s" class="button button-secondary">Configure Payment Methods</a>', 'nostr-login-pay' ),
                    $status_message,
                    $settings_url
                ),
            ),
            'exchange_rate_provider' => array(
                'title' => __( 'Exchange Rate Provider', 'nostr-login-pay' ),
                'type' => 'select',
                'description' => __( 'Choose a provider to convert fiat prices to satoshis', 'nostr-login-pay' ),
                'default' => 'coinbase',
                'desc_tip' => true,
                'options' => array(
                    'coinbase' => __( 'Coinbase', 'nostr-login-pay' ),
                    'bitstamp' => __( 'Bitstamp', 'nostr-login-pay' ),
                    'kraken' => __( 'Kraken', 'nostr-login-pay' ),
                ),
            ),
        );
    }
        ...
        if ( is_user_logged_in() ) {
            ...
            if ( $connection ) {
                ...
                <div class="nwc-payment-info" style="padding: 15px; background: #f0fdf4; border: 1px solid #86efac; border-radius: 6px; margin: 10px 0;">
                    <p style="margin: 0;">
                        <strong style="color: #15803d;">⚡ <?php _e( 'Your Lightning Wallet is Connected', 'nostr-login-pay' ); ?></strong>
                    </p>
                    <p style="margin: 10px 0 0 0; font-size: 14px;">
                        <?php _e( 'Payment will be processed instantly from your connected wallet.', 'nostr-login-pay' ); ?>
                    </p>
                </div>
                <?php
            } else {
                ...
                <div class="nwc-payment-warning" style="padding: 15px; background: #fef3c7; border: 1px solid #fbbf24; border-radius: 6px; margin: 10px 0;">
                    <p style="margin: 0;">
                        <strong><?php _e( 'No Lightning Wallet Connected', 'nostr-login-pay' ); ?></strong>
                    </p>
                    <p style="margin: 10px 0 0 0; font-size: 14px;">
                        <?php _e( 'You need to connect your Lightning wallet first.', 'nostr-login-pay' ); ?>
            ...
            <div class="nwc-payment-notice" style="padding: 15px; background: #eff6ff; border: 1px solid #3b82f6; border-radius: 6px; margin: 10px 0;">
                <p style="margin: 0; font-size: 14px;">
                    <?php _e( 'Please log in and connect your Lightning wallet to use this payment method.', 'nostr-login-pay' ); ?>
                </p>
            </div>
            <?php
        }
    ...
    public function process_payment( $order_id ) {
        ...
        $order->add_order_note( sprintf(
            __( 'Lightning payment initiated. Amount: %d sats (%s %s)', 'nostr-login-pay' ),
            $total_sats,
            $order->get_total(),
        ...
        $order->update_status( 'pending', __( 'Awaiting Lightning payment', 'nostr-login-pay' ) );
    ...
    private function convert_to_satoshis( $amount, $currency ) {
        ...
        if ( ! $btc_price ) {
            return new WP_Error( 'exchange_rate_error', __( 'Could not get Bitcoin exchange rate', 'nostr-login-pay' ) );
        }
    ...
    public function receipt_page( $order_id ) {
        ...
        if ( ! $order || $order->is_paid() ) {
            echo '<p>' . __( 'This order has already been paid.', 'nostr-login-pay' ) . '</p>';
            return;
        }
        
        echo '<p>' . __( 'Please complete your payment using Bitcoin Lightning Network.', 'nostr-login-pay' ) . '</p>';
    ...
    public function thankyou_page( $order_id ) {
        ...
        echo '<h2 class="woocommerce-order-details__title">' . esc_html__( 'Complete Your Payment', 'nostr-login-pay' ) . '</h2>';
    ...
    public function ajax_get_merchant_nwc() {
        ...
        if ( ! empty( $lightning_address ) ) {
            ...
            $order->add_order_note( __( 'Lightning invoice created via LNURL', 'nostr-login-pay' ) );
            $order->save();
    ...
    public function ajax_save_nwc_invoice() {
        ...
        $order->add_order_note( __( 'Lightning invoice created and sent to customer', 'nostr-login-pay' ) );
        $order->save();
    ...
    public function ajax_nwc_mark_paid() {
        ...
        $order->add_order_note( sprintf( 
            __( 'Lightning payment confirmed via %s', 'nostr-login-pay' ), 
            $verification_method 
        ) );
        $order->save();
    ...
    public function ajax_mark_lightning_paid_admin() {
        ...
        // Mark as paid
        $order->payment_complete();
        $order->add_order_note( __( 'Lightning payment confirmed manually by admin', 'nostr-login-pay' ) );
