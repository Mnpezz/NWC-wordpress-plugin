<?php
/**
 * Plugin Name: Nostr Login and Pay
 * Plugin URI: https://github.com/yourusername/nostr-login-and-pay
 * Description: Enable users to login with Nostr and connect their NWC (Nostr Wallet Connect) wallet for payments
 * Version: 1.0.0
 * Author: Your Name
 * Author URI: https://yourwebsite.com
 * License: GPL v2 or later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: nostr-login-pay
...
define( 'NOSTR_LOGIN_PAY_VERSION', '1.0.0' );
...
class Nostr_Login_And_Pay {
    ...
    public function load_textdomain() {
        load_plugin_textdomain( 'nostr-login-pay', false, dirname( NOSTR_LOGIN_PAY_PLUGIN_BASENAME ) . '/languages' );
    }
    ...
    public function enqueue_admin_assets( $hook ) {
        ...
        if ( 'settings_page_nostr-login-pay' !== $hook ) {
            return;
        }

        wp_enqueue_script(
            'nostr-login-pay-admin',
    ...
    public function add_nostr_login_button() {
        ...
        echo '<span>' . esc_html__( 'Login with Nostr', 'nostr-login-pay' ) . '</span>';
        echo '</button>';
    ...
    public function add_nostr_login_button_before_form() {
        ...
        if ( ! is_user_logged_in() ) {
            ...
            echo '<div class="nostr-login-status"></div>';
            echo '</div>';
            echo '<div class="nostr-login-divider"><span>' . esc_html__( 'or', 'nostr-login-pay' ) . '</span></div>';
            echo '</div>';
        }
    }
    ...
    public function add_nostr_login_button_wp_login() {
        ...
        jQuery(document).ready(function($) {
            ...
            var $nostrButton = $('<div class="nostr-login-container" style="margin-bottom: 20px;">' +
                ...
                '<span><?php esc_html_e( 'Login with Nostr', 'nostr-login-pay' ); ?></span>' +
                '</button>' +
                '<div class="nostr-login-status"></div>' +
                '</div>' +
                '<div class="nostr-login-divider" style="margin: 20px 0;"><span><?php esc_html_e( 'or', 'nostr-login-pay' ); ?></span></div>');
    ...
    public function ajax_verify_nostr_login() {
        ...
        if ( ! class_exists( 'Nostr_Login_Pay_Auth' ) ) {
            wp_send_json_error( array( 'message' => __( 'Authentication handler not available', 'nostr-login-pay' ) ) );
        }
        ...
        if ( empty( $pubkey ) || empty( $signature ) || empty( $event ) ) {
            wp_send_json_error( array( 'message' => __( 'Missing required data', 'nostr-login-pay' ) ) );
        }
        ...
        if ( ! $verified ) {
            wp_send_json_error( array( 'message' => __( 'Invalid signature', 'nostr-login-pay' ) ) );
        }
        ...
        wp_send_json_success( array(
            'message' => __( 'Successfully logged in!', 'nostr-login-pay' ),
    ...
    public function ajax_verify_nwc() {
        ...
        if ( ! class_exists( 'Nostr_Login_Pay_NWC_Wallet' ) ) {
            wp_send_json_error( array( 'message' => __( 'NWC wallet handler not available', 'nostr-login-pay' ) ) );
        }

        $nwc_url = isset( $_POST['nwc_url'] ) ? sanitize_text_field( $_POST['nwc_url'] ) : '';

        if ( empty( $nwc_url ) ) {
            wp_send_json_error( array( 'message' => __( 'Missing NWC URL', 'nostr-login-pay' ) ) );
        }
        ...
        wp_send_json_success( array(
            'message' => __( 'NWC wallet connected successfully!', 'nostr-login-pay' ),
